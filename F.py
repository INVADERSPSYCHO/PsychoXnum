...
ssl_context.set_ciphers('DEFAULT@SECLEVEL=1')  # Force older cipher
...
async def fetch(session, num):
    try:
        ...
        async with session.get(API_URL.format(num), timeout=15, ssl=ssl_context) as resp:
            if resp.status == 200:
